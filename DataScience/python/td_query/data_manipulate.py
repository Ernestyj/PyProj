...
class DataManipulate(TeradataQueryBase):
    ...
    def duplicate_rows_from_bad_and_sample_from_good_into_new_table(self, src_db="pp_scratch_risk",
        ...
        # create from src table without data
        self.create_table_from_src_table_schema(src_db, src_table, dest_db, temp_bad_table)

        # get df from src bad table
        df = self.query(query_string='''select * from {}.{} where has_gloss = 1;'''.format(src_db, src_table))
        # duplicate according to weight
        df_a = df[df['amt2'] == 'a-1k']
        df_b = df[df['amt2'] == 'b-5h']
        df_c = df[df['amt2'] == 'c-1h']
        ...
        # insert rows into dest table
        if weight_a > 0:
            self.insert_to_table(pd.concat([df_a] * (weight_a), ignore_index=True), dest_db, temp_bad_table)
        if weight_b > 0:
            self.insert_to_table(pd.concat([df_b] * (weight_b), ignore_index=True), dest_db, temp_bad_table)
        if weight_c > 0:
            self.insert_to_table(pd.concat([df_c] * (weight_c), ignore_index=True), dest_db, temp_bad_table)
        if weight_d > 0:
            self.insert_to_table(pd.concat([df_d] * (weight_d), ignore_index=True), dest_db, temp_bad_table)
        if weight_e > 0:
            self.insert_to_table(pd.concat([df_e] * (weight_e), ignore_index=True), dest_db, temp_bad_table)
        ...
        count_df = self.query(query_string='''select count(*) from {}.{} where has_gloss = 0;'''.format(src_db, src_table))
        ...
        CREATE TABLE {}.{} AS
        (
        SELECT * FROM {}.{} where has_gloss = 0 SAMPLE {}
        ) WITH DATA PRIMARY INDEX (pmt_txnid);
    ...
    def _translate_hyperloop_rule_to_sql(self, rule: str):
        mapping = {
    ...
    def _combine_rules_sql(self, rule_sqls:list):
        result = "(" + rule_sqls[0] + ")"
        for i in range(1, len(rule_sqls)):
            result = " OR ".join([result, "(" + rule_sqls[i] + ")"])
        return result

    def translate_hyperloop_rules_to_sql(self, rules:list):
        rule_sqls = []
        for rule in rules:
    ...
    def update_custom_weight_col_in_table(self, src_db="pp_scratch_risk", src_table='ms_auto_trend_us2_1_3', src_col='PP_MERCH_GROSS_LOSS'):
        ...
        update {}.{} SET weight=(
        case 
        when PP_MERCH_NET_LOSS is null or PP_MERCH_NET_LOSS <= 0.0 then PMT_USD_AMT 
        else PMT_USD_AMT * 2
        end);
        update {}.{} SET weight=1 where weight is null;
